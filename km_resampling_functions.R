...
temp<-NA.CHR.Weighted(time=time,delta=(delta==1),w.n=w.n,w.d=w.d,tpoints.add=tpoints.add,at.points=at.points)
S.KM<-temp$S.KM
...
# Experimental
temp<-NA.CHR.Weighted(time=time1,delta=(delta1==1))
S1.KM<-temp$S.KM
se1.KM<-temp$se.KM
...
# Normal-type
lower<-dhat-1.96*sqrt(var(dhat.star))
upper<-dhat+1.96*sqrt(var(dhat.star))
if(details){ 
cat("Number of resampling draws=",c(draws),"\n")
cat("Restricted means (dhat) at L=",c(L),"\n")
cat("dhat, 95% CI=",c(dhat,lower,upper),"\n")
}
...
temp<-NA.CHR.Weighted(time=time0,delta=(delta0==1),at.points=dpoints)
S0.dpoints<-temp$S.KM
se0.dpoints<-temp$se.KM

temp<-NA.CHR.Weighted(time=time1,delta=(delta1==1),at.points=dpoints)
...
if(draws>0 & get.band==FALSE) return(list(dhat=dhat,m1L=m1.L,m0L=m0.L,m0.star=m.star.0,m1.star=m.star.1,dstar=dhat.star,Zdstar=Zdhat.star,
                                          lower=lower.asy,upper=upper.asy,L=L,Z.asy=Z.asy,pval.asy=pval.asy,se.dhat.star=se.dhat.star,se.dhat=se.dhat))
...
if(draws>0 & get.band==TRUE) return(list(dhat=dhat,m1L=m1.L,m0L=m0.L,m0.star=m.star.0,m1.star=m.star.1,dstar=dhat.star,Zdstar=Zdhat.star,lower=lower.asy,upper=upper.asy,L=L,
